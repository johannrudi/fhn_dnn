...
"""Model for AutoEncoder to house both encoding and decoding NN."""
class AutoencoderModel(nn.Module):
    def __init__(self,
        ...
        Args:
            encoder (int): the encoding neural network
            output_size (int): the decoding neural network
        """
        super().__init__()
    ...
    # used for obtaining the actual lower dimensionality values for training on other NN
    def encode(self, x):
...
from dlkit.opt.train_ae import train_epochs

sys.path.append(os.path.join(os.path.dirname(__file__), '../utils'))
from utils import (ModeKeys, ModelType, load_parameters, save_parameters, update_parameters_from_args)

from data import (load_data, preprocess_features, preprocess_features_noise, preprocess_labels, postprocess_labels,
                  create_dataloader)
from model import (create_denseNet, create_convNet, create_transformerNet, create_autoencoderNet)
...
def run(args, params):
    ...
    # create model
    model = create_autoencoderNet(params)
